...
4. Peraturan Pemerintah Republik Indonesia tentang Perusahaan Daerah Air Minum.
...
- Sistem menyediakan integrasi pembayaran online (Payment Gateway) dengan metode pembayaran Virtual Account (VA) dan QRIS.
...
| FR-14 | Pembayaran Tunai (Loket) | Petugas dapat mencatat pembayaran tagihan pelanggan di loket dan sistem menerbitkan bukti pembayaran. | Tinggi | Petugas |
| FR-14a| Pembayaran Online (Gateway)| Pelanggan dapat membayar mandiri menggunakan Virtual Account (VA) atau QRIS. Sistem menggenerate kode bayar dan otomatis mengupdate status jika sukses. | Tinggi | Pelanggan |
| FR-15 | Deteksi Status Tagihan | Sistem secara otomatis menandai status tagihan pelanggan (Belum Bayar, Pending, Lunas) berdasarkan status di tabel pembayaran. | Tinggi | Sistem |
...
| FR-17 | Cetak Bukti Pembayaran | Sistem dapat mencetak atau mengunduh bukti pembayaran (resi/struk) yang sah bagi pelanggan. | Sedang | Petugas, Pelanggan |
...
| UC-12 | Bayar Tunai (Loket) | Petugas | Merekam transaksi pembayaran tagihan pelanggan secara tunai melalui loket PDAM. | FR-14 |
| UC-12a| Bayar Online (Gateway) | Pelanggan | Pelanggan melakukan pembayaran tagihan secara mandiri lewat Virtual Account/QRIS. | FR-14a, FR-15 |
| UC-13 | Cetak Bukti Pembayaran | Petugas, Pelanggan | Mencetak atau mengunduh bukti pembayaran untuk pelanggan. | FR-17 |
...
|---|---|
| Kode Use Case | UC-12 |
| Nama Use Case | Catat Transaksi Pembayaran Tunai (Loket) |
| Aktor Utama | Petugas |
...
| Alur Normal (Main Flow) | 1. Petugas memilih menu Pembayaran. <br> 2. Petugas mencari data pelanggan berdasarkan nomor pelanggan atau nama. <br> 3. Sistem menampilkan tagihan aktif pelanggan beserta rinciannya. <br> 4. Petugas mengkonfirmasi pembayaran dan memasukkan jumlah uang yang diterima. <br> 5. Sistem memvalidasi dan menyimpan transaksi pembayaran. <br> 6. Sistem memperbarui status tagihan menjadi 'Lunas'. <br> 7. Sistem menampilkan bukti pembayaran yang dapat dicetak. |
| Alur Alternatif | 3a. Apabila pelanggan tidak memiliki tagihan aktif, sistem menampilkan pesan 'Tidak ada tagihan yang perlu dibayar'. <br> 4a. Apabila jumlah pembayaran tidak sesuai, sistem menampilkan peringatan validasi. |
| Kebutuhan Terkait | FR-13, FR-14, FR-15, FR-16, FR-17 |

#### UC-12a: Bayar Tagihan Online (Gateway)
| Atribut | Keterangan |
|---|---|
| Kode Use Case | UC-12a |
| Nama Use Case | Bayar Tagihan Online (VA / QRIS) |
| Aktor Utama | Pelanggan |
| Kondisi Awal (Pre-condition) | Pelanggan dalam keadaan login. Terdapat tagihan aktif berstatus 'Belum Bayar'. |
| Kondisi Akhir (Post-condition) | Tagihan lunas secara otomatis. Sistem mencatat transaksi dari Gateway. |
| Alur Normal (Main Flow) | 1. Pelanggan melihat rincian tagihan di menu Tagihan. <br> 2. Pelanggan mengklik tombol 'Bayar Online'. <br> 3. Sistem menampilkan pilihan metode (Virtual Account, QRIS). <br> 4. Pelanggan memilih metode dan menerima kode/URL bayar (status menjadi 'Pending'). <br> 5. Pelanggan melakukan pembayaran di bank/e-wallet. <br> 6. Sistem menerima webhook (callback) sukses dari Payment Gateway. <br> 7. Sistem merubah status transaksi dan tagihan menjadi 'Lunas'. |
| Alur Alternatif | 5a. Jika pelanggan tidak membayar hingga batas waktu kedaluwarsa, status transaksi menjadi 'Gagal' dan tagihan kembali menjadi 'Belum Bayar'. |
| Kebutuhan Terkait | FR-14a, FR-15, FR-16, FR-17 |
...
| Alur Normal (Main Flow) | 1. Pelanggan memilih menu Pengaduan. <br> 2. Pelanggan mengklik tombol 'Kirim Pengaduan Baru'. <br> 3. Sistem menampilkan formulir pengaduan. <br> 4. Pelanggan mengisi judul, kategori, dan isi pengaduan. <br> 5. Pelanggan mengklik tombol 'Kirim'. <br> 6. Sistem memvalidasi data dan menyimpan pengaduan. <br> 7. Sistem menampilkan konfirmasi bahwa pengaduan berhasil dikirim. |
...
| Kategori | Jumlah |
|---|---|
| Kebutuhan Fungsional (FR) | 27 item |
| Kebutuhan Non-Fungsional (NFR) | 12 item |
| Use Case (UC) | 21 use case |
